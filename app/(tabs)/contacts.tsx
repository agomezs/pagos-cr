...
function ContactRow({ contact, onPress }: { contact: Contact; onPress: () => void }) {
  return (
    ...
    >
      ...
      {contact.notes ? (
        <Text className="text-sm text-gray-400" numberOfLines={1}>{contact.notes}</Text>
      ) : null}
    </Pressable>
  );
}
  ...
  return (
    <SafeAreaView edges={["top"]} className="flex-1 bg-gray-50">
      <View className="px-4 pb-3">
        <Text className="text-xl font-bold text-gray-900">{LABELS.contacts.screenTitle}</Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center py-16 gap-2">
            ...
            <Text className="text-sm text-gray-400">{LABELS.contacts.emptyHint}</Text>
          </View>
        }
